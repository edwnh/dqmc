# -*- coding: utf-8 -*-
"""
Created on Sun May  4 10:42:15 2025
...
# input: rng, it can be changed during the function
# output: random integer with 64 digits
def rand_uint(rng):
    s0 = rng[rng[16]]
    p = (int(rng[16]) + 1) & 15
    rng[16] = p
...
def create_1(file_sim=None, file_params=None, overwrite=False, init_rng=None,
             Nx=8, Ny=8, N_orbital=1, mu=0.0, tp=0.0, t=1, U=-4.0, V=0.5, dt=0.125, L=40,
             nflux=0,
             n_delay=16, n_matmul=8, n_sweep_warm=200, n_sweep_meas=800,
             period_eqlt=8, period_uneqlt=0,
             meas_bond_corr=1, meas_energy_corr=0, meas_nematic_corr=0,
             trans_sym=1):

    assert L % n_matmul == 0 and L % period_eqlt == 0  # check that
    N = Nx * Ny 
    z = 4  # coordinate number

    # maybe change to other solution if U > 0
    a_value = (U/V + np.sign(V) * sqrt((U/V)**2-16))/4
    g_value = U/4 - sqrt((U/4)**2-V**2)
    ...
    # produce the initial random matrix
    if trans_sym:
    ...
    HS_parameter = np.array([-2, -1, 1, 2])
    gamma = np.array([1/4-np.sqrt(6)/12, 1/4+np.sqrt(6)/12,
                         1/4+np.sqrt(6)/12, 1/4-np.sqrt(6)/12]) 
    
    bps_inter = 2  # bonds per site
    N_inter = bps_inter*N  # total bonds in cluster
    bonds_inter = np.zeros((2, N_inter), dtype=np.int32)    
    for iy in range(Ny):
        for ix in range(Nx):
            i = ix + Nx*iy
            iy1 = (iy + 1) % Ny
            ix1 = (ix + 1) % Nx
            bonds_inter[0, i] = i
            bonds_inter[1, i] = ix1 + Nx*iy
            bonds_inter[0, i + N] = i
            bonds_inter[1, i + N] = ix + Nx*iy1
            
    a = np.ones(N_inter) * a_value
    g = np.ones(N_inter) * g_value
    lamda = np.zeros((N_inter, HS_parameter.size))
    exp_lambda = np.zeros((N_inter, HS_parameter.size))
    exp_lambda_a = np.zeros((N_inter, HS_parameter.size))
    Delta_p = np.zeros((N_inter, 4, 4))
    Delta_q = np.zeros((N_inter, 4, 4))
    weight_comparison_matrice = np.zeros((N_inter, 4, 4))
    for n_inter in range(N_inter):
        g_temp = g[n_inter]
        a_temp = a[n_inter]
        lamda[n_inter, :] = np.array([-sqrt(-dt*g_temp*(3+np.sqrt(6))), -sqrt(-dt*g_temp*(3-np.sqrt(6))),sqrt(-dt*g_temp*(3-np.sqrt(6))), sqrt(-dt*g_temp*(3+np.sqrt(6)))])
        exp_lambda[n_inter, :] = np.array([np.exp(-sqrt(-dt*g_temp*(3+np.sqrt(6)))), np.exp(-sqrt(-dt*g_temp*(3-np.sqrt(6)))),
                                      np.exp(sqrt(-dt*g_temp*(3-np.sqrt(6)))), np.exp(sqrt(-dt*g_temp*(3+np.sqrt(6))))])
        exp_lambda_a[n_inter, :] = np.array([np.exp(-a_temp*sqrt(-dt*g_temp*(3+np.sqrt(6)))), np.exp(-a_temp*sqrt(-dt*g_temp*(3-np.sqrt(6)))),
                                         np.exp(a_temp*sqrt(-dt*g_temp*(3-np.sqrt(6)))), np.exp(a_temp*sqrt(-dt*g_temp*(3+np.sqrt(6))))])
        for i1 in range(HS_parameter.size):
            for i2 in range(HS_parameter.size):
                if i2 != i1:
                    Delta_p[n_inter, i1, i2] = np.exp(lamda[n_inter, i1]-lamda[n_inter, i2])-1
                    Delta_q[n_inter, i1, i2] = np.exp(a_temp*(lamda[n_inter, i1]-lamda[n_inter, i2]))-1
                    weight_comparison_matrice[n_inter, i1, i2] = gamma[i1]/gamma[i2]*np.exp(-(lamda[n_inter,i1]-lamda[n_inter,i2])*(1+a_temp))
    
    init_hs = np.zeros((L, N_inter), dtype=np.int32)
    for l in range(L):
        for n in range(N_inter):
            raw = (rand_uint(rng) >> np.uint64(62)) & np.uint64(0b11)  # get 2 digits, from 0 - 4 
            init_hs[l, n] = raw
...
#    delll = np.array((exp_lmbd[map_i]**2 - 1, exp_lmbd[map_i]**-2 - 1))
    ...
    with h5py.File(file_params, "w" if overwrite else "x") as f:
        ...
        # model parameters
        f["params"]["all_state"] = np.array([0, 1, 2, 3])
        f["params"]["N"] = np.array(N, dtype=np.int32)
        f["params"]["Nx"] = Nx
        f["params"]["Ny"] = Ny
        f["params"]["N_inter"] = N_inter
...
#        f["params"]["U"] = U_i
        ...
        f["params"]["a"] = a
        f["params"]["g"] = g
        f["params"]["bonds_inter"] = bonds_inter
        f["params"]["HS_parameter"] = HS_parameter
        f["params"]["gamma"] = gamma
        f["params"]["lamda"] = lamda
        f["params"]["exp_lambda"] = exp_lambda
        f["params"]["exp_lambda_a"] = exp_lambda_a
        f["params"]["weight_comparison_matrice"] = weight_comparison_matrice
        f["params"]["Delta_p"] = Delta_p
        f["params"]["Delta_q"] = Delta_q
...
#        f["params"]["del"] = delll
    ...
    with h5py.File(file_sim, "a" if one_file else "w" if overwrite else "x") as f:
        # simulation state
        params_relpath = os.path.relpath(
            file_params, os.path.dirname(file_sim))
        ...
        if period_uneqlt > 0:
            ...
            f["meas_uneqlt"]["pair_sw"] = np.zeros(
                num_ij*L, dtype=dtype_num)
            if meas_bond_corr:
                f["meas_uneqlt"]["pair_bb"] = np.zeros(
                    num_bb*L, dtype=dtype_num)
                f["meas_uneqlt"]["jj"] = np.zeros(
                    num_bb*L, dtype=dtype_num)
                f["meas_uneqlt"]["jsjs"] = np.zeros(
                    num_bb*L, dtype=dtype_num)
                f["meas_uneqlt"]["kk"] = np.zeros(
                    num_bb*L, dtype=dtype_num)
                f["meas_uneqlt"]["ksks"] = np.zeros(
                    num_bb*L, dtype=dtype_num)
            if meas_energy_corr:
                f["meas_uneqlt"]["kv"] = np.zeros(
                    num_bs*L, dtype=dtype_num)
                f["meas_uneqlt"]["kn"] = np.zeros(
                    num_bs*L, dtype=dtype_num)
                f["meas_uneqlt"]["vv"] = np.zeros(
                    num_ij*L, dtype=dtype_num)
                f["meas_uneqlt"]["vn"] = np.zeros(
                    num_ij*L, dtype=dtype_num)
            if meas_nematic_corr:
                f["meas_uneqlt"]["nem_nnnn"] = np.zeros(
                    num_bb*L, dtype=dtype_num)
                f["meas_uneqlt"]["nem_ssss"] = np.zeros(
                    num_bb*L, dtype=dtype_num)
