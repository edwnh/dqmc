...
def create_1(filename=None, overwrite=False, seed=None,
             Nx=8, Ny=8, muA=0.0, muB=0.0, tA=1.0, tB=1.0,
             nflux=0,
             tperp=1.0, UA=8.0, UB=0.0, dt=0.125, L=96,
             n_delay=16, n_matmul=8, n_sweep_warm=200, n_sweep_meas=1000,
    ...
    map_i[Nx*Ny:] += 1
    degen_i = np.array((Nx*Ny, Nx*Ny), dtype=np.int32)
    num_i = map_i.max() + 1
    assert num_i == degen_i.size

    # 2 site mapping
    map_ij = np.zeros((N, N), dtype=np.int32)
    degen_ij = np.zeros(Nx*Ny*2*2, dtype=np.int32)
    for jo in range(2):
        for jy in range(Ny):
            for jx in range(Nx):
                for io in range(2):
    ...
    # bond definitions
    bps = 5  # bonds per unit cell
    num_b = bps*Ny*Nx  # total bonds in cluster
    ...
    for iy in range(Ny):
        for ix in range(Nx):
            ...
            bonds[0, i + Nx*Ny  ] = i            # i0 = i
            bonds[1, i + Nx*Ny  ] = ix + Nx*iy1  # i1 = i + y
            bonds[0, i + 2*Nx*Ny] = i + Nx*Ny
            bonds[1, i + 2*Nx*Ny] = ix1 + Nx*iy + Nx*Ny
            bonds[0, i + 3*Nx*Ny] = i + Nx*Ny
            bonds[1, i + 3*Nx*Ny] = ix + Nx*iy1 + Nx*Ny
            bonds[0, i + 4*Nx*Ny] = i
            bonds[1, i + 4*Nx*Ny] = i + Nx*Ny


    # 1 bond 1 site mapping
    num_bs = bps*N  # = num_b*N/(Nx*Ny)
                ...
                for jx in range(Nx):
                    for iy in range(Ny):
                        for ix in range(Nx):
                            ...
                            map_bs[j, i] = k
                            degen_bs[k] += 1
    assert map_bs.max() + 1 == num_bs

    # 2 bond mapping
    num_bb = bps*bps*Nx*Ny  # = num_b*num_b/(Nx*Ny)
    ...
    K[:Ny*Nx, :Ny*Nx] -= tA*tij
    K[Ny*Nx:, Ny*Nx:] -= tA*tij
    for i in range(Ny*Nx):
        K[i, i + Nx*Ny] -= tperp
        K[i + Nx*Ny, i] -= tperp
        K[i, i] -= muA
        K[i + Nx*Ny, i + Nx*Ny] -= muB
    Ku = K
    Kd = K
    exp_Ku = expm(-dt * K)
    exp_Kd = expm(-dt * K)
...
#   exp_K = np.array(mpm.expm(mpm.matrix(-dt * K)).tolist(), dtype=np.float64)
    ...
    U_i = np.array((UA, UB), dtype=np.float64)
...
#    exp_lmbd = float(mpm.exp(mpm.acosh(mpm.exp(0.5*float(U*dt)))))
    ...
    with h5py.File(filename, "w" if overwrite else "x") as f:
        ...
        f["metadata"]["model"] = "Hubbard 2 layer (complex)"
        f["metadata"]["Nx"] = Nx
        f["metadata"]["Ny"] = Ny
        f["metadata"]["bps"] = bps
        f["metadata"]["UA"] = UA
        ...
        if period_uneqlt > 0:
            ...
            if meas_bond_corr:
                f["meas_uneqlt"]["pair_bs"] = np.zeros(num_bs*L, dtype=np.complex)
                f["meas_uneqlt"]["pair_sb"] = np.zeros(num_bs*L, dtype=np.complex)
...
def create_batch(Nfiles=1, prefix=None, seed=None, Nx=8, Ny=8, L=96, **kwargs):
    if seed is None:
        seed = int(time.time())
    if prefix is None:
        prefix = str(seed)
    ...
    for i in range(1, Nfiles):
        rand_jump(rng)
        init_rng = rng.copy()
        init_hs = np.zeros((L, Nx*Ny*2), dtype=np.int32)
