...
def calc_A(G, K, m, als=np.logspace(6, 1, 1+20*(6-1)), plot=False):
    """
    ...
    for i, al in enumerate(als):
        us[i], chi2s[i] = calc_A_al(Gavgp, W, Kp, m, al, precalc, us[i-1, :])[1:]

    fit = CubicSpline(np.log(als[als.argsort()]), np.log(chi2s[als.argsort()]))
    k = fit(np.log(als), 2)/(1 + fit(np.log(als), 1)**2)**1.5
    i = k.argmax()
    A = m * np.exp(np.dot(U, us[i, :]))

    dof = len(W) - n_large
    print(f"alpha={als[i]:6f}\tchi2/dof={chi2s[i]/dof:6f}\tA.sum()={A.sum():6f}")

    if plot:
        c2lo, c2hi = scipy.stats.chi2.interval(0.95, dof)
        plt.plot([als[i], als[i]], [chi2s.min(), chi2s.max()], 'r', lw=1)
        plt.plot([als.min(), als.max()], [dof, dof], 'k', lw=1)
        plt.plot([als.min(), als.max()], [c2lo, c2lo], 'k--', lw=1)
        plt.plot([als.min(), als.max()], [c2hi, c2hi], 'k--', lw=1)
        plt.loglog(als, chi2s, '.', ms=3)
        # plt.semilogx(als, k, lw=1)
        plt.xlabel(r"$\alpha$")
        plt.ylabel(r"$\chi^2$")
        # plt.ylim(len(W)*1e-1, len(W)*1e3)
        plt.show()
...
def calc_A_al(G, W, K, m, al, precalc=None, u_init=None):
    ...
    for i in range(max_iter):
        ...
        print(f"reached max iterations {max_iter}. solver probably failed.")
    return A, u, chi2
